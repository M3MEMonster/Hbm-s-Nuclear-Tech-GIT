		...
		if(player.getHeldItem() != null && player.getHeldItem().getItem() == ModItems.coupling_tool) {
			...
			for(EntityRailCarBase neighbor : intersecting) {
				...
				if(closestOwnCoupling != null && closestNeighborCoupling != null) {
					...
					player.addChatComponentMessage(new ChatComponentText(I18nUtil.resolveKey("chat.entity.rail_car.couple", this.hashCode(), closestOwnCoupling.name(), neighbor.hashCode(), closestNeighborCoupling.name())));
