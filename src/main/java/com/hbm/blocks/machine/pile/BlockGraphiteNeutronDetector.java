		...
		if(!world.isRemote) {
			...
			if(tool == ToolType.SCREWDRIVER) {
				...
				if(!player.isSneaking()) {
					...
					player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_neutron.assembly", x, y, z)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD)));
					player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_neutron.flux", pile.lastNeutrons, pile.maxNeutrons)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				}
			}
