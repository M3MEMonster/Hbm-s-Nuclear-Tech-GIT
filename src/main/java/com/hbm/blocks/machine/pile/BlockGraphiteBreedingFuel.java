	...
	public boolean onScrew(World world, EntityPlayer player, int x, int y, int z, int side, float fX, float fY, float fZ, ToolType tool) {
		...
		if(!world.isRemote) {
			...
			if(tool == ToolType.HAND_DRILL) {
				...
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_breeding_fuel.assembly", x, y, z)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD)));
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_breeding_fuel.depletion", pile.progress, pile.maxProgress)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_breeding_fuel.flux", pile.lastNeutrons)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
			}
		}
