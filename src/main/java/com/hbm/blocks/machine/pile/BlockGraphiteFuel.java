	...
	public boolean onScrew(World world, EntityPlayer player, int x, int y, int z, int side, float fX, float fY, float fZ, ToolType tool) {
		...
		if(!world.isRemote) {
			...
			if(tool == ToolType.HAND_DRILL) {
				TileEntityPileFuel pile = (TileEntityPileFuel) world.getTileEntity(x, y, z);
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_fuel.assembly",x, y, z)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD)));
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_fuel.heat", pile.heat, pile.maxHeat)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_fuel.depletion", pile.progress, pile.maxProgress)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				player.addChatComponentMessage(new ChatComponentText(I18nUtil.format("chat.block.graphite_fuel.flux", pile.lastNeutrons)).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
				if((meta & 8) == 8)
					player.addChatComponentMessage(new ChatComponentText(I18nUtil.resolveKey("chat.block.graphite_fuel.rich")).setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_GREEN)));
			}
		}
