...
public class MachineDysonReceiver extends BlockDummyable implements ILookOverlay, ITooltipProvider {
	...
	public void printHook(Pre event, World world, int x, int y, int z) {
		...
		if(receiver.swarmId > 0) {
			text.add(I18nUtil.format("desc.block.dyson_launcher_receiver.id", receiver.swarmId));
			text.add(I18nUtil.format("desc.block.dyson_launcher_receiver.swarm", receiver.swarmCount));
			text.add(I18nUtil.format("desc.block.dyson_receiver.consumer", receiver.swarmConsumers));
			text.add(I18nUtil.format("desc.block.machine.power", BobMathUtil.getShortNumber(energyOutput)));
		} else {
