...
public class MachinePump extends BlockDummyable implements ITooltipProvider, ILookOverlay {
	...
	public void printHook(Pre event, World world, int x, int y, int z) {
		...
		if(pos[1] > 70) {
			text.add(I18nUtil.format("desc.block.pump.altitude", (BobMathUtil.getBlink() ? 0xff0000 : 0xffff00)));
		}

		if(!((TileEntityMachinePumpBase) te).onGround) {
			text.add(I18nUtil.format("desc.block.pump.on_ground", (BobMathUtil.getBlink() ? 0xff0000 : 0xffff00)));
		}
