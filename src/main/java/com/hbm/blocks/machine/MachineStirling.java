	...
	public void printHook(Pre event, World world, int x, int y, int z) {
		...
		if(this != ModBlocks.machine_stirling_creative) {
			...
			if(stirling.heat > maxHeat) {
				text.add(I18nUtil.format("desc.block.stirling.overspeed", (BobMathUtil.getBlink() ? 0xff0000 : 0xffff00)));
			}

			if(!stirling.hasCog) {
				text.add(I18nUtil.format("desc.block.stirling.no_gear", 0xff0000));
			}
		}
