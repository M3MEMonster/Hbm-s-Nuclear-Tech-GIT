...
public class FluidTank implements Cloneable {
	...
	@SideOnly(Side.CLIENT) public void renderTankInfo(GuiInfoContainer gui, int mouseX, int mouseY, int x, int y, int width, int height) {
		if(x <= mouseX && x + width > mouseX && y < mouseY && y + height >= mouseY) {
			...
			if(this.pressure != 0) {
				list.add(EnumChatFormatting.RED + I18nUtil.resolveKey("desc.fluid.trait.pressure", this.pressure ));
				list.add((BobMathUtil.getBlink() ? EnumChatFormatting.RED : EnumChatFormatting.DARK_RED) + I18nUtil.format("desc.fluid.trait.pressurized"));
			}
