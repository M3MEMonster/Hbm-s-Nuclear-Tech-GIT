...
public class FT_Coolable extends FluidTrait {
	...
	public void addInfoHidden(List<String> info) {
		info.add(I18nUtil.format("desc.fluid.coolable.thermal", heatEnergy, amountReq));
		for(CoolingType type : CoolingType.values()) {

			double eff = getEfficiency(type);

			if(eff > 0) {
				info.add(EnumChatFormatting.YELLOW + "[" + type.name + "] " + EnumChatFormatting.AQUA + I18nUtil.format("desc.fluid.coolable.efficiency", ((int) (eff * 100D))));
			}
		}
	}

	public static enum CoolingType {
		TURBINE(I18nUtil.resolveKey("desc.fluid.coolable.type.TURBINE")),
		HEATEXCHANGER(I18nUtil.resolveKey("desc.fluid.coolable.type.HEATEXCHANGER"));
