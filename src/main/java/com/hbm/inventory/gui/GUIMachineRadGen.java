	...
	public void drawScreen(int mouseX, int mouseY, float f) {
		...
		for(int i = 0; i < 12; i++) {
			...
			this.drawCustomInfo(this, mouseX, mouseY, guiLeft + 65, guiTop + 18 + i * 5, 46, 5, new String[] {
					I18nUtil.resolveKey("desc.gui.rad_gen.slot1", (i + 1)),
					I18nUtil.format("desc.gui.rad_gen.slot2",radgen.production[i]),
				I18nUtil.format("desc.gui.rad_gen.slot3",(radgen.maxProgress[i] - radgen.progress[i]), ((radgen.maxProgress[i] - radgen.progress[i]) * 100 / radgen.maxProgress[i]))
			});
		}
	}
