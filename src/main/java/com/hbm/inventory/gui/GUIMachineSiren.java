...
public class GUIMachineSiren extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		if(!siren.getCurrentType().name().equals(TrackType.NULL.name())) {
			...
			this.fontRendererObj.drawString(I18nUtil.format("desc.gui.siren.type", siren.getCurrentType().getType().name()), 46, 40, color);
			this.fontRendererObj.drawString(I18nUtil.format("desc.gui.siren.volume", siren.getCurrentType().getVolume()), 46, 52, color);
		}
	}
