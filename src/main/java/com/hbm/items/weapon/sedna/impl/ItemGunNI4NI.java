...
public class ItemGunNI4NI extends ItemGunBaseNT implements ICustomizable {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean ext) {
		for (String line : I18nUtil.resolveKeyArray("desc.item.NI4NI")){
			list.add(line);
		}
	...
	public void customize(EntityPlayer player, ItemStack stack, String... args) {
		...
		if(args.length == 0) {
			resetColors(stack);
			player.addChatComponentMessage(ChatBuilder.start(I18nUtil.resolveKey("chat.item.NI4NI.reset_color")).color(EnumChatFormatting.GREEN).flush());
			return;
		}

		if(args.length != 3) {
			resetColors(stack);
			player.addChatComponentMessage(ChatBuilder.start(I18nUtil.resolveKey("chat.item.NI4NI.3hex")).color(EnumChatFormatting.RED).flush());
			return;
		}
		...
		try {
			...
			if(dark < 0 || dark > 0xffffff || light < 0 || light > 0xffffff || grip < 0 || grip > 0xffffff) {
				player.addChatComponentMessage(ChatBuilder.start(I18nUtil.resolveKey("chat.item.NI4NI.color_range")).color(EnumChatFormatting.RED).flush());
				return;
			}

			setColors(stack, dark, light, grip);
			player.addChatComponentMessage(ChatBuilder.start(I18nUtil.resolveKey("chat.item.NI4NI.set_color")).color(EnumChatFormatting.GREEN).flush());
