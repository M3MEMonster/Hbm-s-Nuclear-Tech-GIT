...
public class ItemModHealth extends ItemArmorMod {
	...
	public void addInformation(ItemStack itemstack, EntityPlayer player, List list, boolean bool) {
		...
		list.add(color + I18nUtil.format("desc.item.health",(Math.round(health * 10) * 0.1)));
		list.add("");

		if(this == ModItems.black_diamond) {
			list.add(EnumChatFormatting.DARK_GRAY + I18nUtil.resolveKey("desc.item.health.black_diamond"));
			list.add("");
		}
	...
	public void addDesc(List list, ItemStack stack, ItemStack armor) {
		...
		list.add(color + I18nUtil.format("desc.item.health.add", stack.getDisplayName(), (Math.round(health * 10) * 0.1)));
	}
