...
public class ItemModInsert extends ItemArmorMod {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean bool) {
		...
		if(corrosionProtection)
			list.add(EnumChatFormatting.GOLD + I18nUtil.resolveKey("desc.item.insert.corrosionProtection"));
		if(damageMod != 1F)
			list.add(I18nUtil.format("desc.item.insert.damage",(damageMod < 1 ? "-" : "+"), Math.abs(Math.round((1F - damageMod) * 100))));
		if(projectileMod != 1F)
			list.add(I18nUtil.format("desc.item.insert.projectile",Math.round((1F - projectileMod) * 100)));
		if(explosionMod != 1F)
			list.add(I18nUtil.format("desc.item.insert.explosion", Math.round((1F - explosionMod) * 100)));
		if(speed != 1F)
			list.add(I18nUtil.format("desc.item.insert.speed",Math.round((1F - speed) * 100)));
	...
	public void addDesc(List list, ItemStack stack, ItemStack armor) {
		...
		if(damageMod != 1F)
			desc.add(I18nUtil.format("desc.item.insert.damage.add",(damageMod < 1 ? "-" : "+"),Math.abs(Math.round((1F - damageMod) * 100))));
		if(projectileMod != 1F)
			desc.add(I18nUtil.format("desc.item.insert.projectile.add",Math.round((1F - projectileMod) * 100)));
		if(explosionMod != 1F)
			desc.add(I18nUtil.format("desc.item.insert.explosion.add",Math.round((1F - explosionMod) * 100)));
		if(explosionMod != 1F)
			desc.add(I18nUtil.format("desc.item.insert.speed.add",Math.round((1F - speed) * 100)));
