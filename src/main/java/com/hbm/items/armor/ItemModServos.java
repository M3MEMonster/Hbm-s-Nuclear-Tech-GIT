...
public class ItemModServos extends ItemArmorMod {
	...
	public void addInformation(ItemStack itemstack, EntityPlayer player, List list, boolean bool) {
		...
		if(this == ModItems.servo_set) {
			list.add(EnumChatFormatting.DARK_PURPLE + I18nUtil.resolveKey("desc.item.servo_set1"));
			list.add(EnumChatFormatting.DARK_PURPLE + I18nUtil.resolveKey("desc.item.servo_set2"));
		}
		if(this == ModItems.servo_set_desh) {
			list.add(EnumChatFormatting.DARK_PURPLE + I18nUtil.resolveKey("desc.item.servo_set_desh1"));
			list.add(EnumChatFormatting.DARK_PURPLE + I18nUtil.resolveKey("desc.item.servo_set_desh2"));
		}
	...
	public void addDesc(List list, ItemStack stack, ItemStack armor) {
		...
		if(item.armorType == 1) {
			...
			if(this == ModItems.servo_set) {
				list.add(I18nUtil.format("desc.item.servo_set.type1.add",stack.getDisplayName()));
			}
			if(this == ModItems.servo_set_desh) {
				list.add(I18nUtil.format("desc.item.servo_set_desh.type1.add",stack.getDisplayName()));
			}
		}
		...
		if(item.armorType == 2) {
			...
			if(this == ModItems.servo_set) {
				list.add(I18nUtil.format("desc.item.servo_set.type2.add",stack.getDisplayName()));
			}
			if(this == ModItems.servo_set_desh) {
				list.add(I18nUtil.format("desc.item.servo_set_desh.type2.add",stack.getDisplayName()));
			}
		}
	}
