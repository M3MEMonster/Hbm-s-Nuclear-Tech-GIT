...
public class ItemTeleLink extends Item {
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		...
		if(!player.isSneaking() && !world.isRemote) {
			...
			if(!(te instanceof TileEntityMachineTeleporter)) {
				...
				player.addChatMessage(new ChatComponentText(EnumChatFormatting.AQUA + I18nUtil.format("chat.item.tele_link.set_des", x, y, z)));
				player.swingItem();
				...
				if(!stack.hasTagCompound()) {
					...
					player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + I18nUtil.resolveKey("chat.item.tele_link.no_des")));
					return false;
				}
				...
				tele.markDirty();
				world.playSoundAtEntity(player, "hbm:item.techBleep", 1.0F, 1.0F);
				player.addChatMessage(new ChatComponentText(EnumChatFormatting.AQUA + I18nUtil.resolveKey("chat.item.tele_link.has_des")));
				player.swingItem();
				return true;
			}
		}
	...
	public void addInformation(ItemStack itemstack, EntityPlayer player, List list, boolean bool) {
		if (itemstack.stackTagCompound != null) {
			...
			list.add(EnumChatFormatting.RED + I18nUtil.resolveKey("desc.item.tele_link.exit"));
		}
	}
