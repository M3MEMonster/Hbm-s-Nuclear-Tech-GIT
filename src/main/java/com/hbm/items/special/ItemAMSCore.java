...
public class ItemAMSCore extends Item {
	...
	public void addInformation(ItemStack itemstack, EntityPlayer player, List list, boolean bool) {
		...
		if (this == ModItems.ams_core_sing) {
			for (String line : I18nUtil.resolveKeyArray("desc.item.special.ams_core_sing")){
				list.add(line);
			}
		}

		if (this == ModItems.ams_core_wormhole) {
			for (String line : I18nUtil.resolveKeyArray("desc.item.special.ams_core_wormhole")){
				list.add(line);
			}
		}

		if (this == ModItems.ams_core_eyeofharmony) {
			for (String line : I18nUtil.resolveKeyArray("desc.item.special.ams_core_eyeofharmony")){
				list.add(line);
			}
		}
		...
		if (this == ModItems.ams_core_thingy) {
			if(MainRegistry.polaroidID == 11) {
				list.add(I18nUtil.resolveKey("desc.item.special.ams_core_thingy.polar"));
			} else {
				for (String line : I18nUtil.resolveKeyArray("desc.item.special.ams_core_thingy")){
					list.add(line);
				}
			}
		}
