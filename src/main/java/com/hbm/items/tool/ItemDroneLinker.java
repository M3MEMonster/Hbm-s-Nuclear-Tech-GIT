			...
			if(!world.isRemote) {
				if(!stack.hasTagCompound()) {
					...
					player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
							...
							.next(I18nUtil.resolveKey("chat.item.drone_linker.set_pos")).color(EnumChatFormatting.AQUA).flush());
					...
					if(prev instanceof IDroneLinkable) {
						...
						player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
								.nextTranslation(this.getUnlocalizedName() + ".name").color(EnumChatFormatting.DARK_AQUA)
								.next("] ").color(EnumChatFormatting.DARK_AQUA)
								.next(I18nUtil.resolveKey("chat.item.drone_linker.link_set")).color(EnumChatFormatting.AQUA).flush());
					} else {
						player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
								.nextTranslation(this.getUnlocalizedName() + ".name").color(EnumChatFormatting.DARK_AQUA)
								.next("] ").color(EnumChatFormatting.DARK_AQUA)
								.next(I18nUtil.resolveKey("chat.item.drone_linker.link_lost")).color(EnumChatFormatting.RED).flush());
					}
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int slot, boolean inhand) {
		...
		if(world.isRemote && inhand) {
			if(stack.hasTagCompound()) {
				...
				MainRegistry.proxy.displayTooltip(I18nUtil.resolveKey("chat.item.drone_linker.prev_pos", x, y, z), MainRegistry.proxy.ID_DRONE);
			}
		}
	}
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
		...
		if(!world.isRemote && stack.hasTagCompound()) {
			...
			player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
					.nextTranslation(this.getUnlocalizedName() + ".name").color(EnumChatFormatting.DARK_AQUA)
					.next("] ").color(EnumChatFormatting.DARK_AQUA)
					.next(I18nUtil.resolveKey("chat.item.drone_linker.clear_pos")).color(EnumChatFormatting.GREEN).flush());
		}
