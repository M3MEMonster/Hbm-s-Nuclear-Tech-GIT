...
public class ItemTurretControl extends Item {
	...
	public void addInformation(ItemStack itemstack, EntityPlayer player, List list, boolean bool) {
		if(itemstack.stackTagCompound != null) {
			list.add(I18nUtil.resolveKey("desc.item.turret_control.link"));
			...
			list.add(I18nUtil.resolveKey("desc.item.turret_control.select"));
		}
	}
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int fx, float fy, float fz, float f0) {
		...
		if(world.getBlock(x, y, z) instanceof TurretBase) {
			...
			if(!world.isRemote) {
				player.addChatMessage(new ChatComponentText(I18nUtil.resolveKey("desc.item.turret_control.linked")));
			}
