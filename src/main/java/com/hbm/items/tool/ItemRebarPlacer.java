	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float fx, float fy, float fz) {
		...
		if(!hasConk) {
			player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
					...
					.next(I18nUtil.resolveKey("chat.item.rebar_placer.no_type")).color(EnumChatFormatting.RED).flush());
			return true;
		}
		...
		if(!stack.stackTagCompound.hasKey("pos")) {
			...
			if(rebarLeft <= 0) {
				player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
						.nextTranslation(this.getUnlocalizedName() + ".name").color(EnumChatFormatting.DARK_AQUA)
						.next("] ").color(EnumChatFormatting.DARK_AQUA)
						.next(I18nUtil.resolveKey("chat.item.rebar_placer.out")).color(EnumChatFormatting.RED).flush());
				stack.stackTagCompound.removeTag("pos");
				return true;
			}
			...
			player.addChatMessage(ChatBuilder.start("[").color(EnumChatFormatting.DARK_AQUA)
					.nextTranslation(this.getUnlocalizedName() + ".name").color(EnumChatFormatting.DARK_AQUA)
					.next("] ").color(EnumChatFormatting.DARK_AQUA)
					.next(I18nUtil.resolveKey("chat.item.rebar_placer.place", rebarUsed)).color(EnumChatFormatting.GREEN).flush());
