...
public abstract class ItemStructureTool extends Item implements ILookOverlay {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean ext) {
		...
		if(anchor == null)
			list.add(EnumChatFormatting.RED + I18nUtil.resolveKey("desc.item.tool.structure.no_anchor"));

		if(GeneralConfig.enableDebugMode)
			list.add(EnumChatFormatting.GREEN + I18nUtil.resolveKey("desc.item.tool.structure.debug"));
	}
	...
	public void printHook(Pre event, World world, int x, int y, int z) {
		...
		if(anchor == null) {
			text.add(EnumChatFormatting.RED + I18nUtil.resolveKey("desc.item.tool.structure.render.no_anchor"));
		} else {

			int dX = x - anchor.getX();
			int dY = y - anchor.getY();
			int dZ = z - anchor.getZ();
			text.add(I18nUtil.format("desc.item.tool.structure.render.position", dX, dY, dZ));
			...
			if(this.dualUse() && stack.stackTagCompound.hasKey("x")) {
				...
				text.add(I18nUtil.format("desc.item.tool.structure.render.select", sX, sY, sZ));
			}
		}

		if(Minecraft.getMinecraft().thePlayer.isSneaking())
			text.add(I18nUtil.format("desc.item.tool.structure.render.sneak", world.getBlock(x, y, z).getUnlocalizedName(), world.getBlockMetadata(x, y, z)));
