...
public class ItemICFPellet extends Item {
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean bool) {
		...
		list.add(EnumChatFormatting.GREEN + I18nUtil.format("desc.item.ICF_pellet.depletion", String.format(Locale.US, "%.1f", getDurabilityForDisplay(stack) * 100D)));
		list.add(EnumChatFormatting.YELLOW + I18nUtil.format("desc.item.ICF_pellet.fuel", I18nUtil.resolveKey("icffuel." + getType(stack, true).name().toLowerCase(Locale.US)), I18nUtil.resolveKey("icffuel." + getType(stack, false).name().toLowerCase(Locale.US))));
		list.add(EnumChatFormatting.YELLOW + I18nUtil.format("desc.item.ICF_pellet.heat", BobMathUtil.getShortNumber(this.getFusingDifficulty(stack))));
		list.add(EnumChatFormatting.YELLOW + I18nUtil.format("desc.item.ICF_pellet.reac_mult", ((int) (getType(stack, true).reactionMult * getType(stack, false).reactionMult * 100) / 100D)));
		if(muon) list.add(EnumChatFormatting.DARK_AQUA + I18nUtil.resolveKey("desc.item.ICF_pellet.muon"));
